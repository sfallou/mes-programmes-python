...
class Graphique:   
    ...
    def buildMatrix(self):
        ...
        for i in range(n):
            ...
            self.caseB[i] = Entry(self.matrixWindows, textvariable=self.elementMatriceBi, bg ='white', font=("arial", 10), width=3)
    ...
    def solveEquation(self):
        ...
        for i in range(n):
            ...
            try:            
                if type(self.caseB[i]["textvariable"]) is not type(1):
                    error += "\n * Veuillez Remplir les cases par des nombres entiers"     
                    self.footerLabel.config(text=error, fg="red", font="arial 10")                    
                    return 0  # stopping programm"""
            b.append(int(self.caseB[i]["textvariable"]))
            """except:
            ...
            for j in range(n):
                """try:
                    if type(self.caseA[i*n+j]["textvariable"]) is not type(1):
                        error += "\n * Veuillez Remplir les cases par des nombres entiers"     
                        self.footerLabel.config(text=error, fg="red", font="arial 10")                    
                        return 0  # stopping programm"""
                A[i].append(int(self.caseA[i*n+j]["textvariable"]))
                """except:
                    error += "\n * Tous les éléments de la matrice doivent être des nombres"
                    self.footerLabel.config(text=error, fg="red", font="arial 10")                  
                    return 0  # stopping programm"""	
