...
class Canon(object):
	...
	def __init__(self, boss, id, x, y, sens, coul):
		...
		self.corps = boss.create_oval(x -self.rc, y -self.rc, x +self.rc,y +self.rc, fill =coul)
	...
	def test_obstacle(self, xo, yo):
		...
		for id in self.guns:	# id = clef dans dictionn.
			...
			if xo < gun.x1 +self.rc and xo > gun.x1 -self.rc \
				...
				self.anim =False 	# dessiner l'explosion de l'obus (cercle jaune) :
	...
	def fin_explosion(self):
		"effacer l'explosion ; réinitaliser l'obus ; gérer le score"
...
class Pupitre(Frame):
	...
	def __init__(self, boss, canon):
		...
		self.regl.pack(side =LEFT)	# Étiquette d'identification du canon :
