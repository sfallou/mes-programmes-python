...
def quadripartition(px,x1,x2,y1,y2,s):
	if x2-x1<4 or y2-y1<4:
		...
		if esthomogene(px,x1,x2,y1,y2,s):
			...
			quadripartition(px,x1+(x2-x1)//2+1,x2,y1,y1 +(y2-y1)//2,s)
			quadripartition(px,x1,x1+(x2-x1)//2,y1 +(y2-y1)//2+1,y2,s)
			quadripartition(px,x1+(x2-x1)//2+1,x2,y1 +(y2-y1)//2+1,y2,s)		
...
if __name__=="__main__":
	...
	s=100
	t0 = time.clock()
	quadripartition(px,x1,x2,y1,y2,s)
	t1 = time.clock()
	print("temps d'execution en seconde: ",t1-t0)
	#im.show()
	#im.save("resultatSplitSeul.jpg")