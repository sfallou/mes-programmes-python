...
class ThreadConnexion(threading.Thread):
	...
	def run(self):
		...
		while 1:
			...
			print("**{0}** de {1}".format(msgClient, nom))
			deb = msgClient.split(',')[0]
			if deb == "fin" or deb =="":
				self.app.enlever_canon(nom)		# signaler le départ de ce canon aux autres clients :
				self.app.verrou.acquire()
				for cli in self.app.conn_client:
					if cli != nom:
						message = "départ_de,{0}".format(nom)
						self.app.conn_client[cli].send(message.encode("Utf8"))
				self.app.verrou.release()			# fermer le présent thread :
				break
			elif deb =="client OK":				# signaler au nouveau client les canons déjà enregistrés :
				msg ="canons,"
				...
				self.app.verrou.acquire()
				self.connexion.send(msg.encode("Utf8"))	# attendre un accusé de réception ('OK') :
				self.connexion.recv(100).decode("Utf8")
				self.app.verrou.release()				# ajouter un canon dans l'espace de jeu serveur.
					# la méthode invoquée renvoie les caract. du canon créé :
				x, y, sens, coul = self.app.ajouter_canon(nom)		# signaler les caract. de ce nouveau canon à tous les
																	# clients déjà connectés :
				self.app.verrou.acquire()
				for cli in self.app.conn_client:
					msg ="nouveau_canon,{0},{1},{2},{3},{4}".\
						format(nom, x, y, sens, coul)		# pour le nouveau client, ajouter un champ indiquant
				...
				self.app.tir_canon(nom)			# Signaler ce tir à tous les autres clients :
				self.app.verrou.acquire()
				for cli in self.app.conn_client:
					if cli != nom:
						message = "tir_de,{0},".format(nom)
						self.app.conn_client[cli].send(message.encode("Utf8"))
				self.app.verrou.release()
			elif deb =="orienter":
				t =msgClient.split(',')		# on peut avoir reçu plusieurs angles. utiliser le dernier :
				self.app.orienter_canon(nom, t[-1])		# Signaler ce changement à tous les autres clients :
				self.app.verrou.acquire()
				for cli in self.app.conn_client:
					if cli != nom:			# virgule terminale, car messages parfois groupés :
						message = "angle,{0},{1},".format(nom, t[-1])
						self.app.conn_client[cli].send(message.encode("Utf8"))
				self.app.verrou.release()			
								# Fermeture de la connexion :
		self.connexion.close()		    	# couper la connexion
		del self.app.conn_client[nom] 		# suppr. sa réf. dans le dictionnaire
		self.app.afficher("Client %s déconnecté.\n" % nom)	# Le thread se termine ici
...
class AppServeur(AppBombardes):
	...
	def ajouter_canon(self, id):
		...
		coul =('dark blue', 'dark red', 'dark green', 'purple','dark cyan', 'red', 'cyan', 'orange', 'blue', 'violet')[n]
